...
from framework import get_model, get_trainer, get_model_ori
...
def get_processed_data(args, d, val_ratio, test_ratio, df_ratio, subset='in'):
    '''pend for future use'''
    ...
    model1 = None  # common scene: 控制forget set的选择
    data = split_forget_retain(data, df_ratio, subset, model1)
    return data
...
def main():
    ...
    # Dataset
    data = get_processed_data(args, args.dataset, val_ratio=0.05, test_ratio=0.05, df_ratio=args.df_size)
    ...
    # NPO for gnndelete
    # trainer.train(model, data, optimizer, args, logits_ori, attack_model_all, attack_model_sub, model_ori=model_ori)
