...
class DPOTrainer(Trainer):
    ...
    def train_fullbatch(self, model, data, optimizer, args, logits_ori=None, attack_model_all=None, attack_model_sub=None):
        ...
        for epoch in trange(args.epochs, desc='Unlearning'):
            ...
            # DPO: probs_w preference; approximation
            probs_w = model.decode(z, data.train_pos_edge_index[:, ~data.df_mask]).sigmoid()[:sum(data.df_mask)] 
            # probs_l = logits
            beta = 0.5
            # loss = -torch.mean(torch.log(torch.sigmoid(beta * (torch.log(probs_w / ref_l) - torch.log(logits / ref_l)))))
            
            
            # Grad Descent loss on retain set
            rt_logit = model.decode(z, data.train_pos_edge_index[:, data.dr_mask]).sigmoid()
            # GD_loss = - torch.mean(torch.log(logits))
            ...
            # KL-divergence loss on retain set
            rt_label_one_hot = F.one_hot(rt_label.long(), num_classes=2).float()
            rt_logit_expanded = rt_logit.unsqueeze(-1).repeat(1, 2)
            KL_loss = F.kl_div(torch.log(rt_logit_expanded + 1e-8), rt_label_one_hot, reduction='batchmean')
            
            loss = -torch.mean(torch.log(torch.sigmoid(beta * (torch.log(probs_w / ref_l) - torch.log(logits / ref_l))))) + GD_loss
