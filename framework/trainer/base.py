...
class Trainer:
    ...
    def eval(self, model, data, stage='val', pred_all=False):
        ...
        if hasattr(data, 'dtrain_mask'):
            ...
            mask = data.dr_mask
        z = model(data.x, data.train_pos_edge_index[:, mask])       # 为什么不使用完整的图结构data.dtrain_mask？
        ...
        log = {
            ...
            f'{stage}_dt_logit_mean': np.mean(logits.tolist()[:int(len(logit)/2)+1]) if len(logits) > 0 else np.nan,
            f'{stage}_dt_logit_std': np.std(logits.tolist()[:int(len(logit)/2)+1]) if len(logits) > 0 else np.nan
        }
