...
class GNNDeleteNPOTrainer(Trainer):

    def train(self, model, data, optimizer, args, logits_ori=None, attack_model_all=None, attack_model_sub=None, model_ori=None):
        if 'ogbl' in self.args.dataset:
            args.eval_on_cpu = False
            return self.train_fullbatch(model, data, optimizer, args, logits_ori, attack_model_all, attack_model_sub, model_ori)

        else:
            return self.train_fullbatch(model, data, optimizer, args, logits_ori, attack_model_all, attack_model_sub, model_ori)

    def train_fullbatch(self, model, data, optimizer, args, logits_ori=None, attack_model_all=None, attack_model_sub=None, model_ori=None):
        ...
        # GCN as ref
        # './checkpoint/DBLP/gcn/original/42'
        model_ori = model_ori.to(device)
        original_path = os.path.join('./checkpoint', args.dataset, args.gnn, 'original', str(args.random_seed))
        if os.path.exists(os.path.join(original_path, 'model_best.pt')):
            ...
            model_ori.load_state_dict(model_ckpt_sft['model_state'], strict=False)
        # with torch.no_grad():
        #     z1_ref, z2_ref = model_ori(data.x, data.train_pos_edge_index, return_all_emb=True)  
        #     z2_ref_logits = model_ori.decode(z2_ref, data.train_pos_edge_index).sigmoid()
        #     z2_ref_l = z2_ref_logits[data.df_mask]     
        #     z1_ref_logits = model_ori.decode(z1_ref, data.train_pos_edge_index).sigmoid()
        #     z1_ref_l = z1_ref_logits[data.df_mask]  
        ...
        ref_logits = (z2_ori[data.train_pos_edge_index[0]] * z2_ori[data.train_pos_edge_index[1]]).sum(dim=-1)
        ref_l = ref_logits[data.dr_mask][:sum(data.df_mask)] 
        
        z1_ref_logits = (z1_ori[data.train_pos_edge_index[0]] * z1_ori[data.train_pos_edge_index[1]]).sum(dim=-1)
        z1_ref_l = z1_ref_logits[data.dr_mask][:sum(data.df_mask)] 
        ...
        for epoch in trange(args.epochs, desc='Unlearning'):
            ...
            # NPO V1 retain set性能严重下降
            # z1, z2 = model(data.x, data.train_pos_edge_index, return_all_emb=True)
            # z1_logits = model.decode(z1, data.train_pos_edge_index[:, data.df_mask]).sigmoid()
            # z2_logits = model.decode(z2, data.train_pos_edge_index[:, data.df_mask]).sigmoid()
            # beta = 0.5
            # loss_r1 = 2 * torch.mean(torch.log(1 + torch.pow(z1_logits / z1_ref_l + 0.0001, beta))) / beta
            # loss_r2 = 2 * torch.mean(torch.log(1 + torch.pow(z2_logits / z2_ref_l + 0.0001, beta))) / beta  # avoid nan when logits / ref_l = 0
            
            # NPO V2
            z2_logits = model.decode(z2, data.train_pos_edge_index[:, data.df_mask]).sigmoid()
            z1_logits = model.decode(z1, data.train_pos_edge_index[:, data.df_mask]).sigmoid()
            beta = 0.5
            loss_r1 = 2 * torch.mean(torch.log(1 + torch.pow(z1_logits / z1_ref_l + 0.0001, beta))) / beta  # avoid nan when logits / ref_l = 0
            loss_r2 = 2 * torch.mean(torch.log(1 + torch.pow(z2_logits / ref_l + 0.0001, beta))) / beta  # avoid nan when logits / ref_l = 0
            ...
            if self.args.loss_type == 'both_all':
                ...
                loss1 = loss_r1 + 3 * loss_l1
                loss1.backward(retain_graph=True)

                loss2 = loss_r2 + 3 * loss_l2 
    ...
    def train_minibatch(self, model, data, optimizer, args, logits_ori=None, attack_model_all=None, attack_model_sub=None, model_ori=None):
        best_metric = 0
